...
export default function CartItemRow({ item, onQuantityChange, onRemove }: Props) {
  return (
    <div className="flex gap-3 border-b border-border py-4 last:border-b-0 sm:gap-4 sm:py-6">
      {/* Картинка / заглушка */}
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="flex items-start justify-between gap-3">
          ...
          <button onClick={onRemove} aria-label="Remove" className="shrink-0 rounded-full p-1.5 text-text-tertiary transition-colors hover:bg-bg-tertiary hover:text-text">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="mt-auto flex items-center justify-between pt-3 sm:pt-4">
          ...
          <div className="flex items-center rounded-full border border-border">
            <button onClick={() => onQuantityChange(item.quantity - 1)} aria-label="Decrease" className="flex h-8 w-8 items-center justify-center rounded-full text-text-secondary transition-colors hover:text-text">
              <Minus size={14} />
            </button>
            <span className="min-w-8 text-center text-sm font-medium text-text">{item.quantity}</span>
            <button onClick={() => onQuantityChange(item.quantity + 1)} aria-label="Increase" className="flex h-8 w-8 items-center justify-center rounded-full text-text-secondary transition-colors hover:text-text">
              <Plus size={14} />
            </button>
          </div>

          {/* Цена */}
          <span className="text-sm font-semibold text-text sm:text-base">{formatPrice(item.price * item.quantity)}</span>
        </div>
      </div>
    </div>
  );
